# fractal-code-generator

🌀 Fractal Code Generator - Donde el arte fractal se encuentra con el código

## 👤 Autoría del proyecto

**Juan Pablo Benegas** es el autor y creador de este proyecto. Es un
desarrollador autodidacta.

La implementación fue realizada por Juan Pablo Benegas con asistencia de
inteligencias artificiales. No hubo colaboración humana externa en el
desarrollo del proyecto.

## 📄 Licencia

Este proyecto está bajo la **GNU General Public License v3.0 (GPLv3)** 
con una **Excepción Especial** que refleja la filosofía del proyecto.

### ⚖️ Resumen Rápido

| ¿Qué quieres hacer? | Está permitido |
|---------------------|----------------|
| Usarlo personalmente | ✅ Sí |
| Usarlo en tu empresa (internamente) | ✅ Sí |
| Modificarlo y distribuirlo | ✅ Sí (bajo GPLv3) |
| Usarlo en investigación | ✅ Sí |
| Usarlo en arte/educación | ✅ Sí |
| Cerrar el código y no compartir mejoras | ❌ No |

### 📋 Escenarios Detallados

**🏢 Para Empresas**
- ✅ Uso interno permitido
- ✅ No necesitas liberar tus modificaciones
- ✅ Debes reportar bugs y mejoras
- ✅ Debes atribuir el trabajo original
- ❌ No puedes distribuir externamente sin liberar código

**🔬 Para Investigadores**
- ✅ Uso libre en universidades
- ✅ Modificaciones permitidas
- ✅ Debes citar el software en publicaciones
- 🙏 Se recomienda compartir mejoras

**🎨 Para Artistas y Educadores**
- ✅ Uso completamente libre
- ✅ Sin restricciones adicionales
- 🙏 Se agradece la atribución

### 📄 Texto Completo

El texto completo de la licencia está disponible en el archivo [LICENSE](LICENSE) 
e incluye:

1. El texto oficial de la GPLv3 (675 líneas)
2. La Excepción Especial para Fractal Code Generator

### 🛡️ Filosofía de la Licencia

> *"Protegemos nuestro trabajo, pero abrazamos la colaboración. 
> Las empresas pueden experimentar, los investigadores pueden 
> innovar, los artistas pueden crear, y todos contribuyen al 
> ecosistema."*

---

**📧 Para consultas específicas sobre licencias:**

- Juan Pablo Benegas: [benegasjuanpablo265@gmail.com](mailto:benegasjuanpablo265@gmail.com)
- Cuenta de GitHub: [AlambiqueSauron@proton.me](mailto:AlambiqueSauron@proton.me)
